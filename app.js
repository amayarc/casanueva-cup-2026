// ========= App principal =========
const FIREBASE_URL = "https://casanueva-cup-2026-default-rtdb.firebaseio.com";
let DATA = null;
...
function renderClasificacion() {
  ...
  // Tabla completa
  const campBy = {}; (DATA.bonus || []).forEach(b => { campBy[b.slot] = b.campeon; });
  tbody.innerHTML = DATA.clasificacion.map(c => {
    const cls = c.pos <= 3 ? `pos-${c.pos}` : '';
    const camp = campBy[c.slot];
    return `
      <tr class="${cls}">
        <td>${c.pos}</td>
        <td>${escapeHtml(c.nombre)}${camp ? '<div class="clasif-campeon">Campeón: ' + escapeHtml(camp) + '</div>' : ''}</td>
        <td class="total">${c.pts_total}</td>
        <td class="hide-mobile center">${c.pts_partidos}</td>
        <td class="hide-mobile center">${c.pts_bonus}</td>
        <td class="center">${c.marcadores_exactos}</td>
        <td class="center">${c.ganadores_correctos}</td>
      </tr>
    `;
  }).join('');
}
...
function renderBotaActual() {
  const cont = document.getElementById('mundial-bota');
  if (!cont) return;
  const bota = DATA.meta.bota_de_oro;
  if (!bota || !bota.lideres || bota.lideres.length === 0) {
    cont.innerHTML = '<p class="info" style="text-align:center;padding:24px;color:var(--muted);">Aún no hay datos de goleadores. Aparecerán aquí cuando Amaya los cargue.</p>';
    return;
  }
...
function renderMundialBracket() {
    ...
    return `
      <div class="bracket-match${jug ? ' played' : ''}">
        <div class="bracket-team">
          ...
          <span class="bracket-score">${jug ? gl : '—'}</span>
        </div>
        <div class="bracket-team">
  ...
  // ===== Octavos: ganadores de R32 según el cuadro oficial 2026 =====
  ...
  const leftR32 = (R32REAL.length ? R32REAL : allByFase.R32).slice(0, 8);
  const rightR32 = (R32REAL.length ? R32REAL : allByFase.R32).slice(8, 16);
  const leftOCT = (R32REAL.length ? OCT_PAIRS : allByFase.OCT).slice(0, 4);
  const rightOCT = (R32REAL.length ? OCT_PAIRS : allByFase.OCT).slice(4, 8);
...
const ADMIN_PASS = "casanueva2026";
